...
class MyConcurrentSessionControlAuthenticationStrategyOnAuthTests {
    ...
    @Test
    void shouldAllowWhenCurrentSessionExists() {
        SessionRegistry registry = mock(SessionRegistry.class);
        SessionInformation existingSession = new SessionInformation("user", "existing-session", new Date());
        List<SessionInformation> sessions = new ArrayList<>();
        ...
        // Mock session ID to match existing session
        MockHttpServletResponse response = new MockHttpServletResponse();

        // This should not throw because session count equals allowed but current session exists
        assertDoesNotThrow(() -> strategy.onAuthentication(auth, request, response));
    }
